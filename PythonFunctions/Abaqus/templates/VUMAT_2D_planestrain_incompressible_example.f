...
C     ----------------------------------------------------------------
C 	  *** INPUT FROM PYTHON PROGRAM *** MATERIAL PARAMETERS
	  
C
C	  EXAMPLE CASE FOR INPUT PARAMETER SYTAX
C	  ----------------------------------------------------------------
C	  C10 = props(3)
C	  C01 = props(4)
C	  C20 = props(5)
...
C        ---------------------------------------------------------------------------------------------------------------
C		 *** INPUT FROM PYTHON PROGRAM *** DERIVATIVE OF STRAIN-ENERGY FUNCTION
		 
C
C	  	 EXAMPLE CASE FOR INPUT PARAMETER SYTAX
C	  	 ----------------------------------------------------------------		 
C		 dWdI1 = C10 + 2.d0*C20*(I1b - 3.d0)
C		 dWdI2 = C01
C		 dWdJ1 = (2.d0/D1)*(detJ - 1.d0)
C
C        CALCULATE THE COROTATIONAL STRESS 
C        ---------------------------------------------------------------------------------------------------------------
C        sigma_co_ji = 2*(dWdI1+dWdI2*I1b)*B^star_ij - 2 * dWdI2*B^star_ik*B^star_kj - p
C 					 = p1 * B^star_ij - p2 * B^star_ik*B^star_kj - p3
C  		 ---------------------------------------------------------------------------------------------------------------
		 p1 = 2.d0*(dWdI1 + dWdI2 * I1b)
		 p2 = 2.d0*dWdI2
		 p3 = 2.d0*(dWdI1+dWdI2*I1b)*Bbar33 - 2.d0*dWdI2*(Bbar33*Bbar33) 
